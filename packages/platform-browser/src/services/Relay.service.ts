...
export class RelayService {
  ...
  private loadTokenFromStorage(): void {
    try {
      ...
      if (stored) {
        ...
        if (data.expiry && this.now() < data.expiry) {
  ...
  private saveTokenToStorage(token: string, expiry: number): void {
    try {
      const storageKey = `${RelayService.STORAGE_PREFIX}${this.storageKey}`;
      this.storage.setItem(storageKey, JSON.stringify({ token, expiry }));
    } catch (_error) {}
  }
  ...
  async authenticate(): Promise<SIWEAuthResult> {
    try {
      ...
      if (result.success && result.token) {
        ...
        this.authExpiry = this.now() + expiresIn;

        this.saveTokenToStorage(token, this.authExpiry);
      }
  ...
  private isAuthenticated(): boolean {
    ...
    const notExpired = this.authExpiry !== null && this.now() < this.authExpiry;
