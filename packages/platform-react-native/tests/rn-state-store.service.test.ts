...
import RNStateStore from '../src/services/RNStateStore.service';
...
describe('RNStateStore', () => {
  it('supports store lifecycle and CRUD', async () => {
    const store = new RNStateStore({ backend: new InMemoryBackend() });
  ...
  it('returns only string keys from keys()', async () => {
    ...
    expect(keys.some((key) => typeof key !== 'string')).toBe(false);
  });

  it('supports custom rootPrefix', async () => {
    const backend = new InMemoryBackend();
    const store = new RNStateStore({ backend, rootPrefix: 'custom-root' });

    await store.set('a', 'one', 1);
    await backend.put('foreign-root/ignored/key.json', '{"x":1}');

    await expect(store.getAll('a')).resolves.toEqual([1]);
    const stores = await store.listStores();
