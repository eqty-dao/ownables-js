...
const prepareOwnable = (input: any) =>
  new BuilderService({ packageService: packageServiceFor(input.packageService) }).prepareOwnable(
    input
  );
const prepareDossier = (input: any) =>
  new BuilderService({
    packageService: input.packageService,
    fetchFn: input.fetchFn,
    bundleUrl: input.bundleUrl,
  }).prepareDossier(input);
const buildInstantiateMsg = (input: any) =>
  new BuilderService({
    packageService: packageServiceFor({ processPackage: vi.fn() }),
  }).buildInstantiateMsg(input);
const deploy = (adapter: any, params: any) =>
  new BuilderService({
    packageService: packageServiceFor({ processPackage: vi.fn() }),
    deployAdapter: adapter,
  }).deploy(params);
const estimateCost = (input: any) =>
  new BuilderService({
    packageService: packageServiceFor({ processPackage: vi.fn() }),
  }).estimateCost(input);
...
describe('@ownables/builder', () => {
  ...
  it('prepareDossier works through the built package bundle path', async () => {
    ...
    try {
      ...
      const builtBuilder = (await import(
        pathToFileURL(BUILT_INDEX_PATH).href
      )) as typeof BuilderModule;
      const expectedBundlePath = fileURLToPath(
        new URL('./dossier.zip', pathToFileURL(BUILT_INDEX_PATH))
      );
